...
void main() {
  ...
  #pragma sokol @fs ssao_blur_fs
in vec2 v_uv;
out float result;

uniform sampler2D blur_image;
uniform ssao_blur_fs_params {
  float texture_width;
...
void main() {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      result += texture(blur_image, v_uv + offset).r;
    }
  }
  ...
  #pragma sokol @program ssao_blur ssao_blur_vs ssao_blur_fs